    ...
    override suspend fun createChannel(networkId: String, channel: Channel): ApiChannel =
        suspendCancellableCoroutine {
            if (channel.isGroupChannel()) {
                ...
                OpenChannel.createChannel((channel as com.zero.android.models.GroupChannel).toOpenParams()) { openChannel, e ->
                    if (e != null) {
                        logger.e("Failed to create channel", e)
                        it.resumeWithException(e)
                    } else {
            ...
            withSameScope {
                (getChannel(channel) as GroupChannel).let {
                    it.updateChannel(params) { channel, e ->
                        ...
                        else coroutine.resumeWithException(e)
                    }
                }
            }
        }
        ...
        suspendCancellableCoroutine<Unit> { coroutine ->
            withSameScope {
                (getChannel(channel) as GroupChannel).setMyPushTriggerOption(alertType.toOption()) {
                    if (it != null) coroutine.resumeWithException(it)
                    else coroutine.resume(Unit)
                }
            }
        }
